...
class Installer(install):
    ...
    def _create_bin(self):
        source_bin_config = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'fibonacci', 'templates', 'fibstarter')
        target_bin_config = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'fibstarter')

        with open(target_bin_config, "wb") as f_conf:
            f_conf.write(open(source_bin_config).read())
        os.system('chmod +x %s' % target_bin_config)
        print
        print '#'*30
        print '#'*30
        ...
        print
        print 'Example Usage:'
        print 'Start Usage: %s -c ./fib.conf' % target_bin_config
        print 'Help Usage: %s -h' % target_bin_config
